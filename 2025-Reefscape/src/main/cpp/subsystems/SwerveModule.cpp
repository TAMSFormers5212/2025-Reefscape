...
SwerveModule::SwerveModule(int driveMotor, int steerMotor, int absEncoder,
                           double offset)
    ...
    m_driveMotor.Configure(m_driveConfig,
                           SparkBase::ResetMode::kResetSafeParameters,
                           SparkBase::PersistMode::kPersistParameters);
    m_steerMotor.Configure(m_steerConfig,
                           SparkBase::ResetMode::kResetSafeParameters,
                           SparkBase::PersistMode::kPersistParameters);
    ...
    cout << "Swerve Module " << getName(driveMotor) << " initialized correctly"
         << endl;
}

frc::SwerveModuleState
SwerveModule::getState() {  // return current module state

    return {units::meters_per_second_t{m_driveEncoder.GetVelocity()},
            units::radian_t{getAbsolutePosition()}};
}

frc::SwerveModulePosition
SwerveModule::getPosition() {  // get encoder positions
...
void SwerveModule::resetDriveMotor() {  // sets pid, current limit, and encoder
    ...
    m_driveConfig.closedLoop.Pidf(kdP, kdI, kdD, kdFF);
    ...
    m_driveConfig.SetIdleMode(SparkBaseConfig::IdleMode::kBrake)
        .VoltageCompensation(12.0)
        .SmartCurrentLimit(40, 60);

    m_driveConfig.encoder
        .PositionConversionFactor(
            (SwerveModuleConstants::wheelCircumfrenceMeters / 1.0376 /
             (SwerveModuleConstants::driveRatio))
                .value())
        .VelocityConversionFactor(
            (SwerveModuleConstants::wheelCircumfrenceMeters / 1.0376 /
             SwerveModuleConstants::driveRatio / 60_s)
                .value());
...
void SwerveModule::resetSteerMotor() {  // sets pid, current limit, encoder
                                        // position, and encoder conversion
                                        // factor to set values
    ...
    m_steerConfig.SetIdleMode(SparkBaseConfig::IdleMode::kBrake)
        .VoltageCompensation(12.0)
    ...
    // m_steerEncoder.SetPositionConversionFactor(pi2 /
    // SwerveModuleConstants::steerRatio);
...
void SwerveModule::resetSteerEncoder() {  // sets relative steer encoder to
...
SwerveModule::getAbsolutePosition() {  // returns the absolute encoder position
    ...
    // m_absoluteEncoder.GetPositionOffset()) * pi2;
    double x = (m_absoluteEncoder.Get() - encoderOffset);
    if (x < 0) {
        x += 1;
    }

    return (x * pi2);  // shouldn't need to add an offset value because position
                       // offset was set in constructor
}
...
double SwerveModule::getSteerPosition() {  // returns the relative steer encoder
                                           // position
...
std::string SwerveModule::getName(
    int driveMotorID) {  // returns the module name (top left, topright,
...
void SwerveModule::setState(
    ...
    // frc::SwerveModuleState::Optimize(state,
    // units::radian_t(getSteerPosition() + 0.25)); frc::SwerveModuleState
    // stateNew = state; if(state.angle.Degrees().value() < 0) {

    //     // state.angle.Degrees() = state.angle.Degrees() +
    //     units::degree_t(180);
    //      stateNew =
    //      frc::SwerveModuleState{units::velocity::meters_per_second_t{state.speed.value()},
    //      frc::Rotation2d(state.angle.Degrees() + units::degree_t(360))};
    // }
    // else {
    //     stateNew =
    //     frc::SwerveModuleState{units::velocity::meters_per_second_t{state.speed.value()},
    //     frc::Rotation2d(state.angle.Degrees())};
    // }
    ...
    frc::SmartDashboard::PutNumber(
        "STATE SPEED " + getName(m_driveMotor.GetDeviceId()),
        optimizedState.speed.value());
    frc::SmartDashboard::PutNumber(
        "current " + getName(m_driveMotor.GetDeviceId()),
        curAngle.Degrees().value());
    frc::SmartDashboard::PutNumber(
        "absolute " + getName(m_driveMotor.GetDeviceId()),
        getAbsolutePosition());
    frc::SmartDashboard::PutNumber(
        "ABS ENCODER " + getName(m_driveMotor.GetDeviceId()),
        m_absoluteEncoder.Get() - encoderOffset);
    // frc::SmartDashboard::PutNumber("STEER ENCODER " +
    // getName(m_driveMotor.GetDeviceId()), getSteerPosition());
    m_steerController.SetReference(
        (optimizedState.angle.Radians().value() * 7 / 9),
        rev::spark::SparkBase::ControlType::kPosition);
    // frc::SmartDashboard::PutNumber("encoder " +
    // getName(m_driveMotor.GetDeviceId()), adjustedAngle);
    ...
    // curAngle = frc::Rotation2d(curAngle.Degrees() +
    // units::angle::degree_t{90});

    // frc::SmartDashboard::PutNumber("CUR ANGLE " +
    // getName(m_driveMotor.GetDeviceId()), curAngle.Degrees().value()); double
    // delta = std::fmod(std::fmod((optimizedState.angle.Radians().value() -
    // curAngle.Radians().value() + pi), pi2) + pi2, pi2) - (pi);  // NOLINT
    // double temp = optimizedState.angle.Radians().value();
    // if(temp < 0) {
    //     temp += pi;
    // }
    frc::SmartDashboard::PutNumber(
        "OBS ANGLE " + getName(m_driveMotor.GetDeviceId()),
        optimizedState.angle.Degrees().value());
    // double adjustedAngle = temp + curAngle.Radians().value();
    //

    // However, I used setPositionPIDWrappingEnabled(), so I don't think this is
    // needed

    // double adjustedAngle = optimizedState.angle.Radians().value();
    // adjustedAngle = optimizedState.angle.Degrees().value()/360;
    // double adjustedPosition = optimizedState.angle.Degrees().value()/360; //
    // turns it into a circle fraction

    // angle we want to go to
    // frc::SmartDashboard::PutNumber("O " +
    // getName(m_driveMotor.GetDeviceId()), delta);

    if (getName(m_driveMotor.GetDeviceId()) ==
        "top left") {  // put the motor encoder velocity
        // std::cout << getName(m_driveMotor.GetDeviceId()) << " "
        // <<optimizedState.speed.value() << " " <<
        // m_driveMotor.GetAppliedOutput() << " " <<
        // m_driveMotor.GetOutputCurrent() << " " <<
        // m_driveEncoder.GetVelocity() << endl;
    }
}

void SwerveModule::Periodic() {
    // frc::SmartDashboard::PutNumber("velocity " +
    // getName(m_driveMotor.GetDeviceId()), abs(getDriveVelocity() / 12));
    // current angle based on the neo encoder
    frc::SmartDashboard::PutNumber(
        "steer num" + getName(m_driveMotor.GetDeviceId()),
        units::radian_t{getSteerPosition()}.value());
    frc::SmartDashboard::PutNumber(
        "angle " + getName(m_driveMotor.GetDeviceId()), getSteerPosition());
    // print the absolute encoder reading
    frc::SmartDashboard::PutNumber(
        getName(m_driveMotor.GetDeviceId()) + " degrees",
        getAbsolutePosition() / 3.14 * 180);
    // this is the absolute encoder reading minus the position offset
    // frc::SmartDashboard::PutNumber(getName(m_driveMotor.GetDeviceId()) + " o
    // abs", getAbsolutePosition()/pi2);
    frc::SmartDashboard::PutNumber(
        "ABS ENCODER " + getName(m_driveMotor.GetDeviceId()),
        m_absoluteEncoder.Get() - encoderOffset);
    frc::SmartDashboard::PutNumber(
...
void SwerveModule::togglePositionOffset(
    bool toggleOffset) {  // turns on or off the absolute encoder position
                          // offset (TESTING ONLY, DO NOT USE IN COMPETITION)
    if (toggleOffset) {
