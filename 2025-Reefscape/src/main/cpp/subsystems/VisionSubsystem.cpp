...
bool VisionSubsystem::VisionCondition() { return false; }

void VisionSubsystem::Periodic() {
    std::shared_ptr<nt::NetworkTable> table =
        nt::NetworkTableInstance::GetDefault().GetTable("limelight");
    table2 = nt::NetworkTableInstance::GetDefault()
                 .GetTable("limelight")
                 ->GetNumberArray("tid", std::vector<double>(6));
    nt::NetworkTableInstance::GetDefault()
        .GetTable("limelight")
        ->PutNumber("ledMode", ledOn);
    ...
    // // }
    // // else if (id == 11.0 ||id == 12.0 ||id == 13.0 ||id == 14.0 ||id
    // == 15.0 ||id == 16.0){
    // //   goalHeightInches = VisionConstants::stageTagHeight;
    // // }

    // double angleToGoalDegrees =
    // VisionConstants::limelightAngleAboveHorizontal +
    // targetOffsetAngle_Vertical; double angleToGoalRadians =
    // angleToGoalDegrees * (pi / 180.0);

    // // calculate distance
    // double distanceFromLimelightToGoalInches =
    // (VisionConstants::speakerTagHeight -
    // VisionConstants::limelightHeight.value()) / tan(angleToGoalRadians);
    // distance = distanceFromLimelightToGoalInches;
    // // double
    // distanceFromCenterToGoalInches=sqrt(pow(distanceFromLimelightToGoalInches,2)-pow(VisionConstants::limelightHorizontalOffset.value(),2));
    // float KpDistance = -0.1f;  // Proportional control constant for distance
    // float desired_distance = 50;
    // float distance_error =
    // abs(desired_distance-distanceFromLimelightToGoalInches) * KpDistance;
    // // reason the swerve drive doesn't stop moving when trying to align to a
    // certain distance from tag. taking the absolute value of it ensures that
    // it will always be positive meaning the robot wont ever think to change
    // directions
    ...
    //  frc::SmartDashboard::PutNumber("actualdistance",
    //  distanceFromCenterToGoalInches);
    frc::SmartDashboard::PutNumber("id", id);
    double targetOffsetAngle_Horizontal = table->GetNumber("tx", 0.0);
    // double targetOffsetAngle_Horizontal = LimelightHelpers::getTX("")
    double heading_error =
        targetOffsetAngle_Horizontal;  //+VisionConstants::subWooferAngleOffset;//asin(VisionConstants::limelightHorizontalOffset.value()/distanceFromLimelightToGoalInches);
    // pid.SetSetpoint(0);
...
bool VisionSubsystem::isTagPresent() { return tagPresent; }
void VisionSubsystem::setDistanceError(double dist_error) {
    distError = dist_error;
}
int VisionSubsystem::getID() { return tagID; }
double VisionSubsystem::getDistanceError() { return distError; }
double VisionSubsystem::getDistance() { return distance; }
void VisionSubsystem::SimulationPeriodic() {
    // Implementation of subsystem simulation periodic method goes here.
}

double VisionSubsystem::getX() { return table->GetNumber("tx", 0.0); }

double VisionSubsystem::getY() { return table->GetNumber("ty", 0.0); }

double VisionSubsystem::getZ() { return table->GetNumber("tz", 0.0); }